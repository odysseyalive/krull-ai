 ...
 *
 * Visual:
 *   [GPU gauge] 12.4 GB free
 *   [CPU gauge] 22.1 GB free
 *
 * The gauge is a tiny SVG ring that fills clockwise as memory gets
 * used, colored amber when healthy, shifting toward red as free
 * memory drops below 30% / 15% of total.
 *
 * The pill polls /api/system/hardware on mount and then every 15 s
 * so the gauge stays roughly current as ollama loads and unloads
 * models. The server-side detector caches for 10 s so this is
 * cheap.
 */
...
function renderGauge({ used, total }: GaugeInput): SVGElement {
  ...
  track.setAttribute("stroke", "rgba(255,255,255,0.09)");
...
export function HardwarePill(): HTMLElement {
  ...
  function paint(hw: Hardware) {
    el.classList.remove("hardware-pill--gpu", "hardware-pill--cpu", "hardware-pill--err");
    if (
  ...
  void refresh();
  // Background poll so the gauge tracks ollama load/unload.
  const timer = window.setInterval(() => void refresh(), POLL_MS);
  // When the host element is removed (page transitions, etc.), drop
  // the timer to avoid a leaked poll.
